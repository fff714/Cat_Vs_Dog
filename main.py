...
idx_to_class = {v: k for k, v in valid_datasets.class_to_idx.items()}
...
def train_and_valid(model, loss_function, optimizer, epochs=25):
    ...
    for epoch in range(epochs):  # 训练epochs轮
        ...
        # 记录信息
        write_file_path = 'message.txt'
        with open(write_file_path, "a") as file_object:
            file_object.write("Epoch: {:03d}, Training: Loss: {:.4f}, Accuracy: {:.4f}%, \n\t\tValidation: Loss: {:.4f}, "
                              "Accuracy: {:.4f}%, Time: {:.4f}s".format(epoch + 1, avg_valid_loss, avg_train_acc * 100,
                                                                        avg_valid_loss, avg_valid_acc * 100, epoch_end -
                                                                        epoch_start) + '\n')
            file_object.write("Best Accuracy for validation : {:.4f} at epoch {:03d}".format(best_acc, best_epoch) + '\n')
            file_object.write("Best Accuracy for validation : {:.4f} at epoch {:03d}".format(best_acc, best_epoch) + '\n')
